...
from hrms.api.shift_checkout import (
	process_auto_checkout_and_attendance,
	get_employee_shift_status,
	record_overtime,
	record_employee_checkin,
	record_employee_checkout,
...
def get_current_user_info() -> dict:
	...
	user = frappe.db.get_value(
		"User", current_user, ["name", "email", "first_name", "full_name", "user_image", "username"], as_dict=True
	) or {"name": current_user, "email": current_user, "full_name": current_user}

	roles = list(frappe.get_roles(current_user))
	user["roles"] = roles
	role_set = set(roles)

	is_admin = bool(role_set & {"System Manager", "Administrator", "HR Manager"}) or current_user == "Administrator"
	is_candidate = "Candidate" in role_set and not is_admin
	is_employee = not is_admin and not is_candidate
	active_employee = get_current_employee_info()
	has_employee_record = bool(active_employee)
	...
	if is_admin:
		...
		account_type = "admin"
		emp_data = None
	elif has_employee_record or ("Employee" in role_set and "Candidate" not in role_set):
		is_candidate = False
		is_employee = True
		account_type = "employee"
		emp_data = active_employee
	elif "Candidate" in role_set:
		is_candidate = True
		is_employee = False
		...
		is_candidate = False
		is_employee = bool(has_employee_record or "Employee" in role_set)
		account_type = "employee" if is_employee else "user"
		emp_data = active_employee if is_employee else None

	user["is_admin"] = is_admin
	user["is_candidate"] = is_candidate
	user["is_employee"] = is_employee
	user["account_type"] = account_type
	user["employee"] = emp_data
	...
	if is_admin:
		...
		user["employee"] = None  # Strictly None for Candidate
	else:
		user["account_type"] = "employee"
		user["employee"] = get_current_employee_info()
...
def get_current_employee_info() -> dict:
	...
	if not employee:
		employee = frappe.db.get_value(
			"Employee",
			{"personal_email": current_user, "status": "Active"},
			[
				"name",
				"first_name",
				"employee_name",
			...
			"Employee",
			{"company_email": current_user, "status": "Active"},
			[
				"name",
				"first_name",
				"employee_name",
